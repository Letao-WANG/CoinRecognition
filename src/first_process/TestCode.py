...
from util_func.util import get_image_gray, cut_coin_image
...
def jsontest(img_num, first):
    ...
    try:
        ...
        if (first):
            ...
            print('En fait, il y a : ' + str(len(data['shapes'])) + ' pièces\n')
            tmp = np.zeros(20)
            tmp = tmp.astype('str')
...
def determine_piece2(image):
    ...
    coin_res = ["Piece1ctm", "Piece2ctms", "Piece5ctms", "Piece10ctms", "Piece20ctms", "Piece50ctms", "Piece1euro",
                "Piece2euros"]
    ...
    for coin in coin_names:
        comparing_img = check_empty_img("test_images\\" + str(coin))
    ...
    print("choose coin: " + str(coin_res[index]))
...
def determine_piece(deter_image):
    ...
    images_standard = []
    for coin in coin_names:
        image_standard = check_empty_img("test_images\\" + str(coin))
...
def check_empty_img(arg):
    if os.path.exists(str(arg) + ".jpeg"):
        img = cv2.imread(str(arg) + ".jpeg")
    elif os.path.exists(str(arg) + ".jpg"):
        img = cv2.imread(str(arg) + ".jpg")
    elif os.path.exists(str(arg) + ".png"):
    ...
    for a in range(0, 60, 2):
        ...
        if (showResult):
            for i in range(len(images)):
                ...
                # print("length coin_value_liste: " + str(len(coin_value_liste)))
        ...
        number_compare = compare_array(combine1, combine2)
        print("Il y a " + str(number_compare) + " meme piece(s)")
        res += number_compare
