...
def delete_background(image,image_gray):
    image_new = image
...
def resize_image(image_gray,image_color):
    """
    :param image_gray:
    :return:
    """
    ...
    newImg = cv2.morphologyEx(image, cv2.MORPH_OPEN, kernel)
    
    kernel = np.ones((5, 5), np.uint8)
    newImg = cv2.morphologyEx(newImg, cv2.MORPH_CLOSE, kernel)

    #x_nonzero, y_nonzero= np.nonzero(newImg ==0)
    #return image_color[np.min(x_nonzero):np.max(x_nonzero), np.min(y_nonzero):np.max(y_nonzero)]
    return delete_background(image_color,newImg)
